...
class Weather {
    ...
    public function calcWeather($currentTick){
        if($this->canCalculate()){
            ...
            if($this->duration <= 0){
                $duration = mt_rand(min(TableSpoon::$weatherMinTime, TableSpoon::$weatherMaxTime), max(TableSpoon::$weatherMinTime, TableSpoon::$weatherMaxTime));
            ...
            if(($this->weatherNow == self::RAINY_THUNDER or $this->weatherNow == self::THUNDER) and is_int($this->duration / 200)){
                ...
                if(count($players) > 0){
                    ...
                    if(TableSpoon::$enableWeatherLightning){
