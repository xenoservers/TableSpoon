...
use pocketmine\event\{level\LevelLoadEvent, Listener, server\CommandEvent};
...
class EventListener implements Listener {
    ...
    public function onLevelLoad(LevelLoadEvent $ev){
        ...
        if(TableSpoon::$weatherEnabled){
            $lvlWeather->setCanCalculate(($lvl->getName() != TableSpoon::$netherName && $lvl->getName() != TableSpoon::$endName)); // This is: if($lvl->getName() != TableSpoon::$netherName && $lvl->getName() != TableSpoon::$endName){}else{} but shorteded...
        }else{
    ...
    public function onLogin(PlayerLoginEvent $ev){
        ...
        if(TableSpoon::$overworldLevelName != "" && !(TableSpoon::$overworldLevel instanceof Level) && PMServer::getInstance()->getDefaultLevel() instanceof Level){
            $orLvl = PMServer::getInstance()->getLevelByName(TableSpoon::$overworldLevelName);
            if($orLvl instanceof Level){
                TableSpoon::$overworldLevel = $orLvl;
            }else{
                TableSpoon::getInstance()->getLogger()->error("Overworld override Level does not exist. Falling back to default.");
                TableSpoon::$overworldLevel = PMServer::getInstance()->getDefaultLevel();
            }
        }else{
            TableSpoon::$overworldLevel = PMServer::getInstance()->getDefaultLevel();
        }
    ...
    public function onEntityDeath(EntityDeathEvent $ev){
        if(TableSpoon::$dropMobExperience){
    ...
    public function onServerCommand(CommandEvent $ev){
        if(in_array($ev->getCommand(), ["\x76\x65\x72\x73\x69\x6f\x6e", "\x76\x65\x72", "\x61\x62\x6f\x75\x74"])){
            TableSpoon::sendVersion($ev->getSender());
        }
    }
    ...
    public function onInteract(PlayerInteractEvent $ev){
        if(TableSpoon::$instantArmorEnabled){
                    ...
                    if($type !== ArmorTypes::TYPE_NULL){
                        switch($type){
                            case ArmorTypes::TYPE_HELMET:
                                ...
                                if(!TableSpoon::$instantArmorReplace && !$old->isNull()){
                                    $skipReplace = true;
                                    break;
                                }
                        ...
                        if(!$skipReplace){
                            if(!TableSpoon::$instantArmorReplace){
    ...
    public function onGameModeChange(PlayerGameModeChangeEvent $ev){
        if(TableSpoon::$clearInventoryOnGMChange){
    ...
    public function onPlayerDropItem(PlayerDropItemEvent $ev){
        if(TableSpoon::$limitedCreative && $ev->getPlayer()->isCreative()){
            $ev->setCancelled();
        }
    }
    ...
    public function onTeleport(EntityTeleportEvent $ev){
        ...
        if(TableSpoon::$registerDimensions && $frLvl instanceof Level && $toLvl instanceof Level && $frLvl !== $toLvl){
