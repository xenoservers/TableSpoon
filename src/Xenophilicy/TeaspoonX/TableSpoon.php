...
class TableSpoon extends PluginBase {
    ...
    /** @var string */
    public const
      BASE_POCKETMINE_VERSION = "3.0.0", TESTED_MIN_POCKETMINE_VERSION = "3.0.0", TESTED_MAX_POCKETMINE_VERSION = "4.0.0";
    ...
    public function onLoad(){
        ...
        if(Utils::isPhared()){ // unphared = dev
            ...
            if(!isset($meta["builderName"]) || !is_array($meta)){
                $this->getLogger()->error("Only use TeaSpoon Builds from Poggit: https://poggit.pmmp.io/ci/Xenophilicy\TeaspoonX/TeaSpoon/~");
                $this->disable = true;
            ...
            $this->getLogger()->warning("You're using a developer's build of TeaSpoon. For better performance and stability, please get a pre-packaged version here: https://poggit.pmmp.io/ci/Xenophilicy\TeaspoonX/TeaSpoon/~");
        }
    ...
    private function checkConfigVersion(){
        if(Utils::isPhared()){
            ...
            if($ver === null || $ver === false || $ver < self::CONFIG_VERSION){
                ...
                $this->getLogger()->critical("Your configuration file is from a higher version of TeaSpoon! Please update the plugin from https://github.com/Xenophilicy\TeaspoonX/TeaSpoon");
            }
        }
