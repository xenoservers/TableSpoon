...
class ShulkerBox extends Transparent {
    ...
    public function onActivate(Item $item, Player $player = null): bool{
        if(TableSpoon::$shulkerBoxEnabled){
            if($player instanceof Player){
                $t = $this->getLevel()->getTile($this);
                if(!($t instanceof TileShulkerBox)){
                    $t = Tile::createTile(Tile::SHULKER_BOX, $this->getLevel(), TileShulkerBox::createNBT($this));
                }
                if(!$this->getSide(Vector3::SIDE_UP)->isTransparent() || !$t->canOpenWith($item->getCustomName()) || ($player->isCreative() && TableSpoon::$limitedCreative)){
                    return true;
                }
